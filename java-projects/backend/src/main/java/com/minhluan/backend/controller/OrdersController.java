...
public class OrdersController {
    ...
    public ResponseEntity<List<Orders>> getOrderssByuserId(@PathVariable("userId") Long userId) {
        List<Orders> Orderss = OrdersService.getOrderssByuserId(userId);  // Assuming findByuser exists
        if (Orderss.isEmpty()) {
            return null;  // Handle empty user list
        }
        // ... rest of your logic to prepare response (e.g., headers)
        return ResponseEntity.ok().body(Orderss);
    }
}